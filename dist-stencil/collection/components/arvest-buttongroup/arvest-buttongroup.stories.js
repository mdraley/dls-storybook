...
export default {
  ...
  argTypes: Object.assign({}, arvestButtonGroupArgs)
};
const TemplateButtonGroup = ({ buttons, color }) => {
  const arrayWatcher = (array) => array.split(','), mapButtons = (array) => array.map((button) => `<arvest-button-primary  label='${button}' color='${color}'></arvest-button-primary>`), cleanCommasFromStr = (str) => str.split(',').join(''), buttonsArray = arrayWatcher(buttons);
