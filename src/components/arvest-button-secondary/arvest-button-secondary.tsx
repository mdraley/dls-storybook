...
@Component({
  tag: 'arvest-button-secondary',
  styleUrl: 'arvest-button-secondary.css',
  shadow: true
})
    ...
    return (
      <Host>
        <button
          ...
          onMouseLeave={this.setSecondaryColor}
          tabIndex={this.disabled || this.isloading ? -1 : 0}
          class={`${this.disabled || this.isloading ? 'default' : 'button'} ${this.isad ? ' ad' : ''}`}
          type={this.type}
          part="btn-custom"
